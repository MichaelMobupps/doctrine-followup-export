 ...
 *   node --import tsx src/scripts/smoke-competitor.ts --critic   # live + verify the script fix
...
function parseArgs(argv: string[]): Opts {
  const o: Opts = { dry: false, critic: false, maxUsd: 1.0, lintMinRate: 0.6 };
  for (let i = 0; i < argv.length; i++) {
    const a = argv[i];
    if (a === "--dry") o.dry = true;
    else if (a === "--critic") o.critic = true;
    else if (a === "--max-usd") {
      const v = Number(argv[++i]);
      if (Number.isFinite(v)) o.maxUsd = Math.max(0, v);
      else console.warn(`Ignoring invalid --max-usd value; keeping ${o.maxUsd}.`);
    } else if (a === "--lint-min-rate") {
      const v = Number(argv[++i]);
      if (Number.isFinite(v)) o.lintMinRate = Math.max(0, Math.min(1, v));
      else console.warn(`Ignoring invalid --lint-min-rate value; keeping ${o.lintMinRate}.`);
    }
  }
  return o;
}
...
async function main(): Promise<void> {
  ...
  say(`mode: ${opts.dry ? "dry (selector only)" : live ? `live (Flash=${model})` : "selector only (GEMINI_API_KEY not set)"}${opts.critic ? " +critic verify" : ""}`);
  ...
  for (const c of CELLS) {
    ...
    try {
      ...
      // --critic verify: run the deterministic competitor-script lint that now
      // feeds the production rewrite loop. When it flags a Latin brand that
      // should be native, drive ONE rewrite directly on Flash (no ledger write)
      // and re-check, to confirm the catch plus rewrite corrects the script.
      if (opts.critic) {
        const scriptCheck = detectCompetitorScriptViolations(draft.body, c.lang);
        if (!scriptCheck.found) {
          say(`  script check: clean (no Latin brand with a known native form).`);
        } else {
          say(`  script check: flagged ${scriptCheck.matches.join(", ")} — this triggers a rewrite in production.`);
          if (spent < opts.maxUsd) {
            const rewriteBase = getRewriterUserPrompt(ctx, draft, {
              issues: scriptCheck.issues,
              suggestions: scriptCheck.suggestions,
            });
            const rewriteUser = studyBlock ? `${studyBlock}\n\n${rewriteBase}` : rewriteBase;
            const res2 = await geminiGenerateJson({
            ...
            spent += geminiCostUsd(res2.model, res2.usage);
            const rewritten = parseSubjectBody(res2.text);
            const recheck = detectCompetitorScriptViolations(rewritten.body, c.lang);
            say(recheck.found
              ? `  after rewrite: STILL ${recheck.matches.join(", ")} (production runs up to two heal iterations).`
              : `  after rewrite: fixed — competitor names now in native script.`);
            say(`  rewritten body: ${rewritten.body.replace(/\n+/g, " ")}`);
          } else {
            say(`  SKIP rewrite: cost cap reached.`);
          }
        }
      }
    } catch (err) {
  ...
  // Synthetic-leak verification of the deterministic net. The earlier cells
  // depend on Flash leaving a Latin brand to exercise the catch, which it may
  // not do. This plants a Russian draft containing a Latin "Ozon" (must flag,
  // native form Озон) and "Wildberries" (Latin-keep, must not flag), then, when
  // live, drives one no-ledger rewrite to confirm the rewrite renders Озон.
  if (opts.critic) {
    say("[synthetic ru leak] planted draft has Latin 'Ozon' and 'Wildberries'");
    const synthCtx = buildCtx({
      lang: "ru", vertical: "cps", sub: "cps_ecommerce", product: "cps",
      expectBlock: true, company: "ShopNova",
      body: "Hi Alex, following up on the ShopNova cost-per-sale partnership.",
    });
    const plantedBody =
      "Здравствуйте, Алекс. На рынке онлайн-ритейла растет конкуренция со стороны Ozon и Wildberries, поэтому развитие собственных продаж имеет значение. Будете ли вы доступны на следующей неделе?";
    const synth = detectCompetitorScriptViolations(plantedBody, "ru");
    const flaggedOzon = synth.found && synth.matches.includes("ozon→Озон");
    const leftWildberries = !synth.matches.some((m) => m.toLowerCase().startsWith("wildberries"));
    if (flaggedOzon && leftWildberries) {
      say(`  catch: flagged ${synth.matches.join(", ")}, left Wildberries Latin — correct.`);
    } else {
      say(`  catch FAIL: expected ozon→Озон flagged and Wildberries untouched; got ${JSON.stringify(synth.matches)}.`);
      hardError = true;
    }
    if (live && flaggedOzon && spent < opts.maxUsd) {
      try {
        const studyBlock = [buildWriterCompetitorBlock(synthCtx), buildWriterExemplarBlock(synthCtx)]
          .filter((b) => b.length > 0)
          .join("\n\n");
        const rewriteBase = getRewriterUserPrompt(
          synthCtx,
          { subject: "Re: MobUpps partnership for ShopNova", body: plantedBody },
          { issues: synth.issues, suggestions: synth.suggestions },
        );
        const rewriteUser = studyBlock ? `${studyBlock}\n\n${rewriteBase}` : rewriteBase;
        const res3 = await geminiGenerateJson({
          systemParts: [UNTRUSTED_DATA_SYSTEM_CLAUSE, getRewriterSystemPrompt()],
          user: rewriteUser,
          maxOutputTokens: 8192,
          model,
        ...
        spent += geminiCostUsd(res3.model, res3.usage);
        const fixed = parseSubjectBody(res3.text);
        const recheck = detectCompetitorScriptViolations(fixed.body, "ru");
        say(recheck.found
          ? `  after rewrite: STILL ${recheck.matches.join(", ")} (production runs up to two heal iterations).`
          : `  after rewrite: fixed — ${fixed.body.includes("Озон") ? "Озон present, " : ""}no Latin Ozon remaining.`);
        say(`  rewritten body: ${fixed.body.replace(/\n+/g, " ")}`);
      } catch (err) {
        const msg = String((err as { message?: string })?.message ?? err);
        say(`  rewrite skipped: ${msg.slice(0, 100)}`);
      }
    } else if (opts.dry) {
      say("  (dry mode: catch verified offline; run without --dry to see the rewrite)");
    }
    say("");
  }
