...
function DetectionPipeline({ email }: { email: SentEmail }) {
  const steps = [
    ...
    { label: "LABEL", active: email.hasContextLabel },
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {email.labelNames.map((label, i) => (
                      <span
                        ...
                        style={
                          email.matchedContextLabels.includes(label)
                            ? { background: "var(--success-muted)", color: "var(--success)" }
...
export default function ContextEmailInspector() {
  ...
  const meta: SentEmailsMeta = (data as any)?.meta || { total: 0, withContextLabel: 0, inDatabase: 0, wouldBePickedUp: 0 };
  ...
  const filteredEmails = emails.filter(e => {
    ...
    if (filter === "labeled") return e.hasContextLabel;
    if (filter === "unlabeled") return !e.hasContextLabel;
    return true;
  });
  ...
  return (
    <div className="space-y-6">
      ...
      {meta.total > 0 && meta.withContextLabel === 0 && (
        <div
