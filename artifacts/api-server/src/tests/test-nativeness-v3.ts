...
test.test("findForbiddenSingletons: native loanwords are exempt per language", () => {
  // "Budget" is standard German, "incremental" is standard Spanish. Neither is
  // an untranslated anglicism in its language, so neither must flag.
  assert.ok(!findForbiddenSingletons("kein Budget gegen offene Transaktionen", "de")
    .some(s => s.toLowerCase() === "budget"));
  assert.ok(!findForbiddenSingletons("mediciones de incremento incremental reales", "es")
    .some(s => s.toLowerCase() === "incremental"));
  assert.ok(!findForbiddenSingletons("medición de incrementalidad real sobre esa audiencia", "es")
    .some(s => s.toLowerCase() === "real"));
  assert.ok(!findForbiddenSingletons("das an einem kleinen Segment zu testen", "de")
    .some(s => s.toLowerCase() === "segment"));
  assert.ok(findForbiddenSingletons("echte Performance fuer ShopNova", "de")
    .some(s => s.toLowerCase() === "performance"));
  // Industry terms kept in English across every language.
  assert.ok(!findForbiddenSingletons("campa\u00f1as de retargeting para la app", "es")
    .some(s => s.toLowerCase() === "retargeting"));
  assert.ok(!findForbiddenSingletons("campanhas de remarketing", "pt")
    .some(s => s.toLowerCase() === "remarketing"));
  assert.ok(!findForbiddenSingletons("\u6211\u4eec\u7684 retargeting \u65b9\u6848", "zh")
    .some(s => s.toLowerCase() === "retargeting"));
  // The exemption is per word and per language: a real anglicism still flags.
  assert.ok(findForbiddenSingletons("wir liefern echte Performance fuer Sie", "de")
    .some(s => s.toLowerCase() === "performance"));
});

test.test("findXNotY: Spanish verb negation is not a contrast", () => {
  // "no hay problema" means "no problem", a benign negation, not an X-not-Y.
  assert.equal(findXNotY("si el momento no es el indicado, no hay problema, queda el contexto", "es").length, 0);
  // A real ", no <noun>" contrast still fires.
  assert.ok(findXNotY("socios de rendimiento, no instalaciones puras", "es").length > 0);
});
...
test.test("findUntransliteratedGreetingName: transliterated name spares deep brands and acronyms", () => {
  // Observed Gemini Pro drafts wrongly flagged before the fix: the prospect
  // name is already in local script, and a brand name plus an acronym appear
  // deeper in the same line as body content.
  const zh = findUntransliteratedGreetingName(
    "您好，亚历克斯，再次跟进我之前关于 ShopNova 的 CPS 合作方案的邮件。",
    "zh",
  );
  assert.deepEqual(zh, []);
  const he = findUntransliteratedGreetingName(
    "שלום אלכס, אני כותב בהמשך להודעה שלי בנושא שיווק מחדש ב-Wanderly Travel.",
    "he",
  );
  assert.deepEqual(he, []);
});

test.test("findUntransliteratedGreetingName: a real Latin name in the slot still fires", () => {
  // The fix must not weaken the real catch.
  assert.deepEqual(findUntransliteratedGreetingName("您好，Alex，再次跟进。", "zh"), ["Alex"]);
  assert.deepEqual(findUntransliteratedGreetingName("שלום Alex, בהמשך להודעה שלי.", "he"), ["Alex"]);
});
