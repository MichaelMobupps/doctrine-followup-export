...
export default function ContextPipeline() {
  ...
  // through to GET /api/context/followups with NO userId filter — the server
  // then returned EVERY user's rows with full email bodies. Only legitimate
  // on a legacy single-user install (no connected accounts). See pipeline.tsx.
  ...
  useEffect(() => {
    ...
    async function fetchFollowups() {
      try {
        ...
        const q = qs.toString();
        const url = `${base}api/context/followups${q ? `?${q}` : ""}`;
