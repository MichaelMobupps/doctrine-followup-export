...
const DE_BAD = "kann ungef\u00e4hr 250 Conversions pro Tag liefern und starke Performance";
...
test.describe("language-nativeness lint", () => {
  ...
  test.it("does NOT flag per-Thai-guide-allowed singletons in the real Thai email", () => {
    const r = detectNativenessViolations(THAI_REAL, "th");
    const singletons = ["cohort", "acquire", "event", "convert", "campaign", "LTV", "UA", "email"];
    for (const s of singletons) {
      assert.ok(!r.matches.includes(s),
        `did not expect "${s}" to be flagged but it was; matches=${JSON.stringify(r.matches)}`);
    }
  });
  ...
  test.it("is a no-op for Latin-script target languages (English)", () => {
    const r = detectNativenessViolations(EN_CLEAN, "en");
    assert.equal(r.found, false);
  });

  test.it("is a no-op for unknown language tags", () => {
    const r = detectNativenessViolations(EN_CLEAN, "xx");
    assert.equal(r.found, false);
  });
...
test.describe("combined detector", () => {
  ...
  test.it("merges doctrine + nativeness reports on Thai email", () => {
    const r = detectAllDeterministicViolations(THAI_REAL, "th");
    assert.equal(r.found, true);
    // Thai email is short and clean re: numbers, so only the nativeness check
    // should fire. Doctrine should be silent.
    assert.match(r.issues.join(" "), /LATIN-TOKEN-LEAK/);
  });

  test.it("doctrine + nativeness silent on clean English", () => {
    const r = detectAllDeterministicViolations(EN_CLEAN, "en");
    assert.equal(r.found, false);
  });
