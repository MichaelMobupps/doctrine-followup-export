...
export type ThreadInboundKind = "none" | "reply" | "bounce";
 ...
 *   - 'reply'  if there is a genuine inbound message from someone other than
 *     the sender that is NOT a bounce (a human reply, or an out-of-office).
 *   - 'none'   if the thread only contains the sender's own messages.
 *
 * Bounce wins over reply: if a thread contains both a daemon NDR and some
...
export async function classifyThreadInbound(
  ...
  for (const msg of messages) {
    ...
    // Skip the sender's own outbound messages.
    if (senderLower && fromLower.includes(senderLower)) continue;
    ...
    // Inbound from a non-sender that is not a bounce: a reply (or an OOO,
    // which proves the address is live and is handled by the reply path).
    sawReply = true;
  }

  return { kind: sawReply ? "reply" : "none" };
}
...
function isFromSender(fromHeader: string, senderEmail: string): boolean {
  ...
  return sender.length > 0 && from.includes(sender);
}
...
export async function detectManualFollowupSend(params: {
  ...
  const outbound = (res.data.messages || [])
    ...
    .filter(({ msg, sentAt }) => {
      ...
      const from = getHeaderValue(msg.payload?.headers, "From");
      return isFromSender(from, params.senderEmail);
    })
