// B9b.8/B9b.9: AntiGhostingPipeline — pipeline view for anti_ghosting prospects.
//
...
// Mirror of api-server effectiveFollowupCap / HARD_FOLLOWUP_CAP. Clamp the
// stored value the same way the scheduler does so a stale max_followups never
// shows "/4" and a capped-out campaign correctly registers as done. Keep in
// sync with HARD_FOLLOWUP_CAP in api-server/src/lib/followupLimits.ts.
...
export default function AntiGhostingPipeline() {
  ...
  const { user: currentUser, identityStatus } = useCurrentUser();

  // Admin pipeline user picker. Defaults to "my pipeline". Renders only with
  // an API key. `effectiveUserId` is the id sent into the list call.
  ...
  // through to GET /api/anti-ghosting/followups with NO userId filter — the
  // server then returned EVERY user's rows with full email bodies. Only
  // legitimate on a legacy single-user install. See pipeline.tsx.
  ...
  // B9b.11: Multi-select state for bulk actions.
  ...
  useEffect(() => {
    // 2026-07-16 main-screen-hang fix: identity gate — never issue the
    // userId-less all-users variant unless this is a legacy install.
    if (!apiKey || identityBlocked) return;
    let cancelled = false;
    ...
    async function fetchFollowups() {
      try {
        ...
        const q = qs.toString();
        const url = `${base}api/anti-ghosting/followups${q ? `?${q}` : ""}`;
  ...
  }, [apiKey, effectiveUserId, refetchTrigger, showArchived, identityBlocked]);

  // When the viewed manager changes, blank the current rows immediately so the
  // list shows its loading state during the refetch rather than rendering the
  // previous manager's rows under the new (banner) name. Keyed on the
  // effective user id only — a poll/mutation refetch must NOT blank the list.
  useEffect(() => {
    setFollowups(null);
    setIsLoading(true);
  }, [effectiveUserId]);
  ...
  // B9b.12.3: bulk send-now mirroring Doctrine. Calls /followup/send-bulk.
  ...
  async function handleBulkPause() {
    ...
    try {
      ...
      showFeedback(
        `Paused ${data.paused || 0} of ${data.total || ids.length}; cancelled ${data.cancelled_queued || 0} queued follow-up${(data.cancelled_queued || 0) !== 1 ? "s" : ""}`,
      );
      clearSelection();
      invalidateAll();
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : "Bulk pause failed";
      showFeedback(msg, true);
    } finally {
  ...
  async function handleBulkResume() {
    ...
    try {
      ...
      showFeedback(
        `Resumed ${data.resumed || 0} of ${data.total || ids.length}; queued ${data.queued || 0} new stage${(data.queued || 0) !== 1 ? "s" : ""}`,
      );
      clearSelection();
      invalidateAll();
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : "Bulk resume failed";
      showFeedback(msg, true);
    } finally {
  ...
  }, [threads, filterState, showArchived]);
  ...
  return (
    <div className="space-y-6">
      ...
      {/* B9b.11: Select-all toggle (visible when threads are listed) */}
      ...
      {identityBlocked && identityStatus === "unmatched" ? (
        // 2026-07-16 main-screen-hang fix: no connected Gmail account for
        // this login — explain instead of fetching every user's rows.
        ...
        // Identity resolution still in flight: skeletons, no fetch issued.
          ...
          {filteredThreads.map((thread, idx) => {
            ...
            return (
              ...
              >
                ...
                >
                  {/* B9b.11: Selection checkbox */}
                  <button
                    type="button"
                    onClick={(e) => {
                      e.stopPropagation();
                      toggleSelected(thread.prospect_id);
                    }}
                    ...
                    title={selectedIds.has(thread.prospect_id) ? "Deselect" : "Select for bulk action"}
                  >
                  ...
                  <div className="flex items-center gap-1 flex-shrink-0" onClick={e => e.stopPropagation()}>
                    ...
                    <>
                    {/* B9b.12.4: show pause/resume toggle on replied prospects too — manual re-engagement. */}
                    {!thread.all_done && (
                      <Button
                        variant="ghost" size="sm"
                    ...
                    {/* B9b.12.3: + always visible (mirrors Doctrine). Backend force-sends regardless of state. */}
                    <Button
                      variant="ghost" size="sm"
                ...
                {isExpanded && (
                  <div className="px-4 pb-4" style={{ borderTop: "1px solid var(--border-default)" }}>
                    ...
                    {/* B9b.12.3: detail panel — Send-now always visible, pause/resume state-gated (mirrors Doctrine). */}
                    <div className="flex items-center gap-2 mt-3 pt-3" style={{ borderTop: "1px solid var(--border-default)" }}>
                      ...
                      {/* B9b.12.4: show Resume on any paused (including replied). */}
                      {thread.followup_paused && (
                        <Button
                          variant="ghost" size="sm"
      ...
      {/* B9b.11: Floating bulk-action bar; appears when 1+ rows are selected. */}
      {selectedIds.size > 0 && (
        <div
        ...
        >
          ...
          {/* B9b.12.3: Send all button + updated disabled props on existing buttons. */}
          <Button
            size="sm"
