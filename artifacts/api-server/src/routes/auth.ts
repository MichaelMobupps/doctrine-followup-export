...
router.post("/auth/exchange", async (req: Request, res: Response) => {
  try {
    ...
    // The verified email is the one stored at the OAuth callback (metadata),
    // NOT anything the client sent. Decide admin status here, the single
    // trustworthy moment. resolveAdminGrant fails closed: if ADMIN_API_KEY is
    // unset, or the email is not in ADMIN_EMAILS, no admin token is issued and
    // the adminToken field is omitted entirely (never an empty string).
    const verifiedEmail = codeData.metadata;
    ...
    // Normal login is unchanged: every user still gets { token, email }.
    const responseBody: {
      token: string;
      email: string | null;
      isAdmin: boolean;
    ...
    if (grant.isAdmin && grant.adminToken) {
      responseBody.adminToken = grant.adminToken;
    }

    if (grant.isAdmin) {
      // Log that an admin token was issued, WITHOUT logging the token itself.
      logger.info({ email: verifiedEmail }, "Admin token issued at exchange");
    }
