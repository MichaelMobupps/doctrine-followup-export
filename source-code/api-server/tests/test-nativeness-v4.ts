...
// ESM-compatible __dirname. The live api-server runs in ESM scope under tsx,
// where the CommonJS __dirname global is unreliable; derive it from
// import.meta.url. From src/tests this resolves correctly to the test dir,
// so ../services points at src/services where the prompt files live.
...
function loadFollowupPromptsText(): string {
  ...
  const dirHint = TEST_DIR;
  const candidates = [
    // Primary: relative to the test file (src/tests -> src/services).
    path.join(dirHint, "..", "services", "followupPrompts.ts"),
    // cwd-based fallbacks for ad-hoc invocation from various roots.
    path.join(cwd, "src", "services", "followupPrompts.ts"),
    path.join(cwd, "services", "followupPrompts.ts"),
    path.join(cwd, "api-server", "src", "services", "followupPrompts.ts"),
