...
export default function Prospects() {
  ...
  const renderStatusBadge = (status?: string | null, scheduled?: string | null) => {
                    ...
                    {group.prospects.map((p: any) => {
                      ...
                      <TableRow key={p.id} className="group">
                        ...
                        {Array.from({ length: 10 }, (_, i) => {
                          ...
                          return <TableCell key={stage} className="text-center">{renderStatusBadge(p[`followup_${stage}_status`], p[`followup_${stage}_scheduled`])}</TableCell>;
                        })}
