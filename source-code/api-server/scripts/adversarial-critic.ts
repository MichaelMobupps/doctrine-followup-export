/**
 * Adversarial end-to-end test for the critic.
 *
 ...
 *
 *   bad draft  ->  critic flags it  ->  rewriter fixes it
 *              ->  critic confirms the rewrite is clean
 *
 * Both stages run through the production waterfalls (lib/modelPolicy.ts), so
 * this measures what production would actually do, including a fallback tier if
 * the primary is at capacity while it runs.
 *
 * Run from artifacts/api-server with GEMINI_API_KEY and OPENAI_API_KEY set:
 ...
 * violation was missed, 2 every critic tier was unavailable (a vendor capacity
...
const cases: Case[] = [
  ...
  {
    // v4 control: generated by the live writer chain against THIS ctx and
    // verified structurally (no LLM grading it) to match both the ship gate
    // and the seeded layout directive ("tight-soft", pattern [1,3], soft
    // break) the critic judges against. The v3 control was real pipeline
    // output but matched only the ship gate, not the directive, so the critic
    // kept flagging it — correctly. The shape is seeded from (company,
    // subject, stage), so this control is valid only for this exact fixture
    // ctx; regenerate rather than hand-write if the doctrine prompt or the
    // layout profiles change.
    name: "clean_control",
    bad: false,
    subject: SUBJECT,
    body:
