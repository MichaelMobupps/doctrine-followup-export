...
function resolveThinking(): ThinkingLevel {
  const v = (process.env.GEMINI_WRITER_THINKING || "MEDIUM").toUpperCase();
  return v === "LOW" || v === "HIGH" ? (v as ThinkingLevel) : "MEDIUM";
}
...
async function genGemini(model: string, systemParts: string[], userPrompt: string): Promise<TierGen> {
  const res = await geminiGenerateJson({ systemParts, user: userPrompt, maxOutputTokens: 8192, model, thinkingLevel: resolveThinking() });
...
async function runCellCompare(cell: Cell, selfTest: boolean, hooks: RunHooks): Promise<CellResult> {
  const ctx = cell.ctx;
  ...
  const plannedChain = planWriterChain({ provider: getWriterProvider(), greyArea: grey, geminiConfigured: geminiOk });
  ...
  for (const tier of TIER_ORDER) {
    ...
    try {
      ...
      if (selfTest) {
        ...
        gen = await genGemini(model, systemParts, userPrompt);
      }
...
async function main(): Promise<void> {
  ...
  const perCellCalls = geminiOk ? 3 : 1;
  ...
  if (!geminiOk) {
    console.log("WARN  GEMINI_API_KEY is not set. The Gemini models will be skipped and only");
    console.log("      Sonnet will produce emails. Set GEMINI_API_KEY to compare all three.\n");
  }
  ...
  async function worker(): Promise<void> {
    for (;;) {
      ...
      const r = await runCellCompare(cells[i], opts.selfTest, hooks);
      results[i] = r;
      for (const t of r.tiers) {
        tally.doneCalls++;
  ...
  const tiers: WriterTier[] = ["gemini_primary", "gemini_secondary", "anthropic"];
