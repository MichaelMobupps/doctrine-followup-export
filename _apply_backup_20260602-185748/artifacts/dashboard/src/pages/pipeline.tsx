...
function groupByThread(followups: FollowupRow[]): EmailThread[] {
  ...
  return threads.sort((a, b) => {
    // B7x: chronological "what did I just send" ordering. The previous
    // sort prioritised whichever active prospect had the soonest next
    // follow-up firing — useful as a worklist, confusing as a pipeline
    // view. A freshly-sent email landed deep in the list because its
    // F1 was 6+ days out while older actives had F2/F3 about to fire.
    //
    // New order, simpler:
    //   1. Done/replied at the bottom (closed campaigns shouldn't squat
    //      at the top of an active pipeline view).
    //   2. Everything else by original_sent_at DESC — newest at top.
    //      Active and paused rows interleave; the state pill on the
    //      row tells the user which is which.
    //   3. Tiebreak by current_stage DESC so same-day prospects show
    //      the more-progressed one first.
    const aClosed = a.replied || a.all_done;
    const bClosed = b.replied || b.all_done;
...
export default function Pipeline() {
  ...
  const { data: followups, isLoading, isError } = useGetFollowups(
    ...
    { ...requestOpts, query: { queryKey: ["/api/followups", effectiveUserId], enabled: !!apiKey, refetchInterval: 30000 } }
  );
  ...
  }, [threads, filterState, filterVertical, showArchived]);
