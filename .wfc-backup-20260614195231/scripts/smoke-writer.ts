...
// Stress preset: the cells where Gemini Pro JSON failures actually appeared,
// plus close neighbors in non-Latin and right-to-left scripts where long Pro
// reasoning is most likely to emit a stray character. cps and retargeting are
// the two verticals that failed. Intentionally small so a Pro-only run is a
// handful of calls.
const STRESS_CELLS: Array<[string, string, number]> = [
  ["ja", "cps", 2],          // failed (malformed JSON)
  ["ja", "retargeting", 2],  // failed (trailing content)
  ["de", "cps", 2],          // failed (malformed JSON)
  ["es", "retargeting", 3],  // failed (trailing content)
  ["zh", "cps", 2],          // CJK neighbor
  ["ko", "retargeting", 2],  // CJK neighbor
  ["ar", "cps", 2],          // RTL neighbor
  ["he", "retargeting", 2],  // RTL neighbor
  ["ru", "cps", 2],          // Cyrillic neighbor
  ["th", "retargeting", 2],  // complex-script neighbor
];
