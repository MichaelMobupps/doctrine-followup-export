...
function getFollowupCap(maxFollowups?: number | null): number {
  // CB-2: rigid cap. See lib/followupLimits.ts.
...
router.get("/prospects", async (req: Request, res: Response) => {
  try {
    ...
    // on multi-user installs (SELECT * incl. full original bodies, 50k cap).
...
router.post("/prospect/:id/pause", async (req: Request, res: Response) => {
  ...
  try {
    ...
    await db.update(prospectsTable).set({ followupPaused: true, pauseReason: "manual_intervention", pausedAt: new Date() }).where(and(SCOPE, eq(prospectsTable.id, prospectId)));
...
router.post("/prospect/:id/resume", async (req: Request, res: Response) => {
  ...
  try {
    ...
    await db.update(prospectsTable).set({ followupPaused: false, pausedAt: null }).where(and(SCOPE, eq(prospectsTable.id, prospectId)));
...
// ====================================================================
// POST /context/prospect/:id/restore — un-archive a campaign
// ====================================================================
// Brings an archived campaign back into the active pipeline view in its
// current paused state and restarts the 14-day archival clock. Does not
// change followup_paused/pause_reason/replied; the operator uses Resume to
// reactivate sending. SCOPE-gated for defense-in-depth.
router.post("/prospect/:id/restore", async (req: Request, res: Response) => {
  ...
  try {
    const result = await db
      ...
      .where(and(SCOPE, eq(prospectsTable.id, prospectId), eq(prospectsTable.archived, true)));
...
router.post("/prospect/pause-bulk", async (req: Request, res: Response) => {
  ...
  try {
    ...
    await db.update(prospectsTable).set({ followupPaused: true, pauseReason: "manual_intervention", pausedAt: new Date() }).where(and(SCOPE, inArray(prospectsTable.id, contextIds)));
...
router.get("/campaign/status", async (_req: Request, res: Response) => {
  try {
    ...
    for (const p of actionableProspects) {
      ...
      if (nextStage > userMaxFollowups) continue;
