...
router.get("/prospects", async (req: Request, res: Response) => {
  ...
  let followupMap: Record<number, Record<number, { status: string; scheduledAt: Date | null }>> = {};
  ...
  if (prospectIds.length > 0) {
    ...
    for (const f of followups) {
      ...
      followupMap[f.prospectId][f.stage] = { status: f.status, scheduledAt: f.scheduledAt };
    }
  }
...
router.post("/followup-now/:prospectId", async (req: Request, res: Response) => {
  ...
  try {
    ...
    const usedStages = new Set(
      existingFollowups
        .filter((f) => ["sent", "queued", "generating", "pending_approval"].includes(f.status))
        .map((f) => f.stage)
    );

    let nextStage = 1;
    while (usedStages.has(nextStage)) nextStage++;
    ...
    try {
      ...
      res.status(400).json({ error: `Stage ${nextStage} already exists` }); return;
    }
    ...
    res.json({
      ...
      message: `Test follow-up stage ${nextStage} queued and processing now.`,
    });
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    res.status(500).json({ error: msg });
