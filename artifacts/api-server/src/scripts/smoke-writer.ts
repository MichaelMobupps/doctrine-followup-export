...
async function runCellCompare(cell: Cell, selfTest: boolean, hooks: RunHooks, only: WriterTier[]): Promise<CellResult> {
  ...
  const userPrompt = studyBlock ? `${studyBlock}\n\n${base}` : base;
...
// Chain mode: drive the REAL production writer chain (runWriter) once per cell
// and record the single tier that actually served the draft. This is what
// answers "which model does production route to" from live behavior, rather
// than the compare mode that calls every model. Usage is captured locally for
// the report via a deps override and is NEVER written to the followup_usage
// ledger, so a smoke run does not affect production billing.
async function runCellChain(cell: Cell, selfTest: boolean, hooks: RunHooks): Promise<CellResult> {
  const ctx = cell.ctx;
  const grey = isGreyArea(ctx);
  const greySignals = detectGreyArea(ctx).signals;
  const exemplarBlock = buildWriterExemplarBlock(ctx);
  ...
  if (hooks.capReached()) {
    result.tiers.push({
      tier: productionRoute, model: modelForTier(productionRoute), status: "SKIP",
      subject: "", body: "", captured: null, lintPass: false, lintIssues: [], ms: 0,
      error: "cost cap reached",
    });
    return result;
  }

  // Self-test: no provider call. Assume the planned first tier serves.
  if (selfTest) {
    const tier = productionRoute;
    const model = modelForTier(tier);
    const captured =
      tier === "anthropic"
        ? anthropicCaptured({ input_tokens: 3200, output_tokens: 240 }, model)
        : geminiCaptured({ promptTokenCount: 3200, candidatesTokenCount: 240, thoughtsTokenCount: 60 }, model);
    const body = fakeBody(ctx, tierLabel(tier));
    const lint = lintBody(body, ctx);
    hooks.addCost(captured.costUsd);
    result.tiers.push({
      tier, model, status: "OK", subject: `Re: ${ctx.original_subject}`, body,
      captured, lintPass: lint.pass, lintIssues: lint.issues, ms: Date.now() - t0, error: null,
    });
    return result;
  }

  // Live: run the real chain. Capture the served tier's cost via deps overrides.
  let captured: Captured | null = null;
  const anthropicWriter: AnthropicWriterFn = async () => {
    const g = await genSonnet(userPrompt);
    captured = g.captured;
    return { subject: g.subject, body: g.body, modelUsed: g.captured.model, tier: "anthropic" };
  };
  try {
    const served = await runWriter(
      { label: "draft", greyArea: grey, systemParts, userPrompt, maxOutputTokens: 8192, prospectName: ctx.prospect_name },
      anthropicWriter,
      {
        // Capture Gemini usage locally; do NOT touch the production ledger.
        recordGeminiUsage: async (usage: GeminiUsageMetadata, model: string) => {
          captured = geminiCaptured(usage, model);
        },
      },
    );
    const cap = captured ?? geminiCaptured({ promptTokenCount: 0, candidatesTokenCount: 0 }, served.modelUsed);
    hooks.addCost(cap.costUsd);
    const lint = lintBody(served.body, ctx);
    const out: TierOutput = {
      tier: served.tier, model: served.modelUsed, status: "OK", subject: served.subject,
      body: served.body, captured: cap, lintPass: lint.pass, lintIssues: lint.issues,
      ms: Date.now() - t0, error: null,
    };
    if (cap.model && MODEL_PRICES[cap.model] === undefined) {
      out.status = "FAIL";
      out.error = `no price row for ${cap.model}`;
      result.hardFailReasons.push(`chain served ${cap.model} with no price row`);
    }
    result.tiers.push(out);
  } catch (err) {
    const msg = String((err as { message?: string })?.message ?? err);
    const status = isOverload(msg) ? "BLOCK" : "FAIL";
    result.tiers.push({
      tier: productionRoute, model: modelForTier(productionRoute), status,
      subject: "", body: "", captured: null, lintPass: false, lintIssues: [], ms: Date.now() - t0,
      error: msg.slice(0, 200),
    });
    if (status === "FAIL") result.hardFailReasons.push(`chain threw: ${msg.slice(0, 140)}`);
  }

  // Same free routing checks as compare mode.
  if (grey && productionRoute !== "anthropic") {
    result.hardFailReasons.push(`grey cell production route is ${productionRoute}, expected anthropic`);
  }
  if (!grey && geminiOk && productionRoute !== "gemini_primary") {
...
function buildLog(results: CellResult[], opts: Options, summary: string): string {
  ...
  out.push("same competitor block and exemplar block. The original outreach is English; the");
  out.push("follow-up is written in original_language. production_route is the model production");
  out.push("would actually use for this cell. Usage is captured for this report and is not written to the ledger.");
  out.push("");
  out.push("---------------------------------------------------------------------------");
...
async function main(): Promise<void> {
  ...
  // Chain mode serves exactly one tier per cell (the real production route);
  // compare mode calls every selected tier.
  const perCellCalls = opts.mode === "chain" ? 1 : runTiers.length;
  ...
  async function worker(): Promise<void> {
    for (;;) {
      ...
      const r =
        opts.mode === "chain"
          ? await runCellChain(cells[i], opts.selfTest, hooks)
          : await runCellCompare(cells[i], opts.selfTest, hooks, opts.only);
      results[i] = r;
      for (const t of r.tiers) {
        tally.doneCalls++;
