...
const ACK_MARKERS: Record<string, string[]> = {
  ...
  de: ["ich komme zuruck auf", "meine vorherige", "meine letzte e-mail", "meine letzte email", "ich hatte geschrieben", "ankupfend an", "bezugnehmend auf", "meine nachricht", "in bezug auf meine"],
  ...
  zh: ["跟进", "上次", "之前", "再次联系", "我之前", "上一封", "跟進", "之前发的", "之前發的", "续上次"],
  ...
  he: ["בהמשך", "חוזר אליך", "המייל הקודם", "המייל הקודם שלי", "הודעתי הקודמת", "כתבתי לך", "פניתי אליך", "במענה ל"],
...
function hasAckMarker(body: string, markers: string[]): boolean {
  const head = body.slice(0, 320).toLowerCase();
  return markers.some((m) => head.includes(m.toLowerCase()));
}
