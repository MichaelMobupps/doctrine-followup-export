...
export function Layout({ children }: LayoutProps) {
  ...
  // 2026-07-29: paused_by_admin rides along on all three /my/activity
  // endpoints. When true, auto-queue is skipping this account entirely —
  // synced emails will never enter the pipeline — so the sidebar must say
  // so instead of "All caught up".
  const [activity, setActivity] = useState<{ queued: number; next_due: string | null; paused_by_admin?: boolean } | null>(null);
  ...
  // Gate the Admin Activity entry on real admin status. A normal manager
  // never sees it. The server still enforces the boundary regardless.
  const visibleNavItems = isAdmin
    ? navItems
    : navItems.filter((item) => item.name !== "Admin Activity");
    ...
    <div className="flex min-h-screen w-full" data-product={dataProduct}>
      ...
      >
        ...
        <div className="p-3 mt-auto space-y-2">
          ...
          >
            ...
            {activity && (
              <div className="text-[10px] space-y-0.5" style={{ color: "var(--text-tertiary)" }}>
                ...
                {activity.queued === 0 && !activity.paused_by_admin && (
                  <p>All caught up</p>
                )}
              </div>
            )}
          ...
          {/* 2026-07-29: admin-pause warning. Without this the account looked
              healthy ("All caught up") while every synced email silently
              skipped the queue. */}
          {activity?.paused_by_admin && (
            <div
              className="rounded-lg p-3"
              style={{ background: "var(--warning-muted)", border: "1px solid var(--warning-border)" }}
            >
              <p className="text-[11px] font-semibold uppercase tracking-[0.04em] mb-1" style={{ color: "var(--warning)" }}>
                Paused by admin
              </p>
              <p className="text-[10px] leading-relaxed" style={{ color: "var(--text-secondary)" }}>
                Follow-ups are not being queued or scheduled for this account.
                Synced emails will not appear in the Pipeline until the
                account is resumed.
                {/* 2026-07-29: only admins can see the Admin Activity page
                    (visibleNavItems strips it), so don't send non-admins
                    hunting for a nav item that doesn't exist. */}
                {isAdmin
                  ? " Resume it from the Admin Activity page."
                  : " Contact your administrator to resume it."}
              </p>
            </div>
          )}
