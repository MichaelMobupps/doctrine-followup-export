...
export async function runStartupMigrations(): Promise<void> {
  try {
    ...
    logger.info(
      "B7r/B7u/B9a + bounce/archive migrations applied (followup_usage, users.paused_by_admin, prospects.cycle/parent_prospect_id/pause_reason/bounce_type/paused_at/archived/archived_at, followups.cycle + unique-constraint swap, users.anti_ghosting_label, thread_messages, app_settings, partial indexes)",
    );
  } catch (err) {
