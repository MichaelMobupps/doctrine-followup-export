...
async function partitionAndSchedule(
  ...
  if (testIds.length > 0) {
    const testSchedule = generateBatchSchedule(testIds, stage, { testMode: true });
...
router.post("/followup-now/:prospectId", async (req: Request, res: Response) => {
  ...
  try {
    ...
    const maxExistingStage = existingFollowups.length > 0
      ? Math.max(...existingFollowups.map((f) => f.stage))
      : 0;
    const nextStage = maxExistingStage + 1;
    const maxFollowups = user[0].maxFollowups || 10;
    if (nextStage > maxFollowups) {
      res.status(400).json({
        error: `Maximum follow-up stage reached (${maxFollowups}). Increase max follow-ups in account settings to queue more.`,
      });
      return;
    }
    ...
    if (!insertResult.rowCount || insertResult.rowCount === 0) {
      res.status(400).json({ error: `Could not queue next stage (${nextStage}). Please retry.` });
      return;
    }
    ...
    res.json({
      ...
      message: `Follow-up stage ${nextStage} queued and processing now.`,
    });
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    res.status(500).json({ error: msg });
...
router.post("/prospect/:id/resume", async (req: Request, res: Response) => {
  ...
  try {
    ...
    const user = p.userId
      ...
      : null;
    const maxFollowups = user?.maxFollowups || 10;
...
router.get("/campaign/status", async (_req: Request, res: Response) => {
  try {
    ...
    for (const p of actionableProspects) {
      ...
      const userMaxFollowups = users.find(u => u.id === p.userId)?.maxFollowups || 10;
...
router.post("/campaign/queue", async (req: Request, res: Response) => {
  ...
  try {
    ...
    for (const prospect of unrepliedProspects) {
      ...
      const userMaxFollowups = users.find(u => u.id === prospect.userId)?.maxFollowups || 10;
...
router.post("/campaign/launch", async (req: Request, res: Response) => {
  ...
  try {
    ...
    const maxStageLimit = (parsedMaxStage && !isNaN(parsedMaxStage) && parsedMaxStage > 0) ? parsedMaxStage : 10;
    const now = new Date();
    ...
    for (const prospect of unrepliedProspects) {
      ...
      const nextStage = (info?.maxSentStage || 0) + 1;
      if (nextStage > maxStageLimit) continue;
