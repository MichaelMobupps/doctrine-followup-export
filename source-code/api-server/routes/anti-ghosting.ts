...
import { effectiveFollowupCap } from "../lib/followupLimits";
// 2026-07-16 main-screen-hang fix: unbounded-list guard for /followups.
...
router.get("/candidates", async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    // 2. Resolve threads that already have an anti_ghosting prospect.
    ...
    // 3. List label-tagged threads. 2026-07-29: per-label union (OR) —
    // a single threads.list with several labelIds is an AND filter and
    // silently matched nothing once sublabels existed. alreadyMarked is
    // passed as the skip set so ingested threads don't consume the cap.
    const threads = await listThreadsWithAnyLabel(user.gmail, labelIds, maxResults, {
    ...
    for (const t of threads) {
      ...
      try {
        ...
        // 2026-07-29: on validator rejection the seed is null, but the thread
        // was usually read just fine. Surface enough context (last message's
        // subject/snippet + an explicit readError flag) for the dashboard to
        // render a proper "not eligible: <reason>" card instead of the
        // misleading "Could not read thread <id>" it showed for EVERY
        // rejection — which is what made label detection look broken.
        const lastMessage = outcome.threadMessages[outcome.threadMessages.length - 1];
        candidates.push({
          gmailThreadId: t.id,
          ok: outcome.ok,
          ...
          readError: outcome.threadMessages.length === 0,
          subject: outcome.seed?.subject ?? lastMessage?.subject ?? null,
          snippet: outcome.seed?.snippet ?? lastMessage?.snippet ?? null,
          seed: outcome.seed
            ? {
...
router.get("/followups", async (req: Request, res: Response) => {
  try {
    ...
    // on multi-user installs — every user's rows with full email bodies in
    // one res.json() froze the requesting tab and the server's event loop.
    if (await rejectUnboundedList(req, res, ["userId", "email"])) return;

    const conds = [eq(prospectsTable.app, "anti_ghosting")];
    if (req.query.includeArchived !== "1") conds.push(eq(prospectsTable.archived, false));
...
// ====================================================================
// POST /api/anti-ghosting/prospect/:id/restore
// ====================================================================
// Un-archive a campaign: brings it back into the active pipeline view in
// its current paused state and restarts the 14-day archival clock. Leaves
// followup_paused/pause_reason/replied untouched; the operator uses Resume
// to reactivate. Scope-gated to app='anti_ghosting'.
router.post("/prospect/:id/restore", async (req: Request, res: Response) => {
  const id = parseInt((req.params.id as string));
  if (isNaN(id)) { res.status(400).json({ error: "Invalid prospect ID" }); return; }
  try {
    const result = await db
      .update(prospectsTable)
      .set({ archived: false, archivedAt: null, pausedAt: new Date() })
      .where(and(eq(prospectsTable.app, "anti_ghosting"), eq(prospectsTable.id, id), eq(prospectsTable.archived, true)));
    res.json({ success: true, restored: Boolean(result.rowCount) });
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    logger.error({ err, prospectId: id }, "POST /anti-ghosting/prospect/:id/restore failed");
    res.status(500).json({ error: msg });
  }
});
...
router.get("/my/activity", async (req: Request, res: Response) => {
  ...
  try {
    ...
    // 2026-07-29 admin-pause visibility — see routes/doctrine.ts /my/activity.
  ...
  try {
    ...
    for (const item of items) {
      ...
      try {
        ...
        // 2026-07-29 direction-hardening: SENT-label-aware, matching sync.
...
router.post("/sync", async (req: Request, res: Response) => {
  try {
    ...
    // internal perUser diagnostics array that must not leak into the API.
    res.json({ synced: result.synced, repliesDetected: result.repliesDetected, auto_queued: autoQueued });
  } catch (err) {
    // Honors err.statusCode (e.g. 409 SyncAlreadyRunningError, 404 unknown user).
...
function getFollowupCapAG(maxFollowups?: number | null): number {
  // CB-2: rigid cap. See lib/followupLimits.ts. AntiGhosting queueing also
  // enforces its own per-cycle cap of 3 independently of this value.
